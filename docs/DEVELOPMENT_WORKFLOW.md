...
- 测试临时目录清理使用 `tools/cleanup_task_temp.ps1`：默认只生成路径、大小与 SHA256 清单；确认属于本任务且已有删除授权后，使用同一清单执行 `-Apply`。仅允许本项目 `temp/qa/` 的严格子目录，拒绝根目录、重解析点、范围外路径以及发生变化的文件集。原生 PowerShell 逐文件删除后再移除空目录，复核目标不存在；记录精确逻辑删除字节与独立测量的磁盘空闲变化。保留清单、回执和必要运行证据。自动审批拒绝时记录原始原因并停止，不切换工具绕过；用户重新明确授权后，重新盘点再执行。真实账号目录、正式交付和历史任务不纳入默认清理范围。
...
- 新游戏或尚未批准目标分辨率的游戏，竖屏默认使用 `720 x 1280`，横屏默认使用 `1280 x 720`；已有项目的正式分辨率优先，迁移不得批量覆盖，必须逐项目完成运行时验收。
- 本项目固定竖屏，Godot viewport 与默认桌面窗口统一为 `720 x 1280`；保留 `canvas_items`、`expand`、固定竖屏、UI 坐标和触控换算。`540 x 960`、`1080 x 1920` 等尺寸仅作为兼容性边界，不再是默认窗口。
- 分辨率验证必须读取真实 GUI 客户区尺寸和当前屏幕可用区，在默认分辨率的一帧内同时看到页面顶部与底部，并保留与声明尺寸一致的运行截图。
