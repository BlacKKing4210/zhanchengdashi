...
## 13. 2026-09-06 B 版运行时集成

- 制作人已明确选择 B「无描边＋浅投影」，并授权资源实装；此决定取代第 11、12 节的待选状态。
- 独立 UI 组件与原生输入控件应用奶油底、浅投影和低饱和度色系；大厅段位板统一；导航只有两种背景状态；卡牌浅色抬起选中和小指示标记；属性、技能、进度居中。实际卡牌、数值和描述继续读取正式配置，不采用效果图的示意数据。
- 战斗移除固定棋盘外框及遮罩，原始六边形 PNG 铺入全屏世界层，HUD 在其上绘制；同步扩大裁剪和相机反算范围，并阻止 HUD 点击、拖动起点和滚轮穿透。
- 小地图开局在 HUD 可视区域内居中，大地图开局保留己方基地焦点并限制初始空白；不改变地图、寻路、射程或默认 1.30 镜头倍率。长塔名保留完整名称，等级独立用同款徽章显示。
- 备份分支：`codex/pre-ui-art-20260906`，包含原有未提交数值工作簿；实现不修改数值表。
- 剩余：B 图标整板的两次生成都返回了带棋盘格的 RGB 图片，未冒充透明资源接入。等待制作人授权本地抠图；主体皮肤、地块和加载插图可独立验收，不宣称整套资源全部完成。
