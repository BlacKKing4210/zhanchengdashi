...
玩家在局外选择出战卡牌，进入局内后通过购买相邻地块、翻开建筑或资源、生成单位和防御建筑来推进领土。双方单位自动沿地块推进，摧毁对方基地或在倒计时结束时靠领土和建筑分数取胜。
...
| 流程节点 | 当前状态 | 说明 |
| --- | --- | --- |
| 大厅 | 已实装 | 中央装饰场景，显示当前编组预览，点击 `开始战斗` 进入战斗 |
...
当前战斗原型在 `main.gd` 中硬编码一组上下镜像地块，尚未完全使用 `board_rules.csv` 的随机地图规则。
...
- 单位进入无建筑地块后可以推进占领进度。
- 如果同一地块同时存在双方单位，占领进度会回退。
- 占领必须满足与己方领土连接。
- 占领速度约为 `delta * 单位数量 * 0.65`。
- 占领完成后地块归属改变，并播放反馈。
...
| 建筑 | HP | 功能 |
| --- | ---: | --- |
| base | 520 | 基地，攻击附近敌人，被摧毁则直接失败/胜利 |
| barracks | 170 | 周期生成单位 |
| hall | 210 | 周期生成强单位 |
...
| 建筑 | 周期/冷却 | 范围 | 伤害 | 说明 |
| --- | ---: | ---: | ---: | --- |
| tower | 0.95 秒 | 150 | 22 | 自动攻击最近敌方单位 |
| base | 1.15 秒 | 175 | 18 | 自动攻击最近敌方单位 |
| barracks - 兔兵 | 3.8 秒 | - | - | 玩家默认周期，敌方默认更快 |
| barracks - 狼兵 | 4.4 秒 | - | - | 周期生成狼兵 |
| barracks / hall - 熊兵 | 5.8 秒 | - | - | 周期生成熊兵 |
...
1. 扫描附近敌方单位。
2. 如果目标在攻击范围内，按冷却造成伤害。
3. 如果目标不在攻击范围内，向目标地块移动。
4. 如果当前地块有敌方建筑，则攻击建筑。
5. 如果没有目标，则向敌方基地移动。
6. HP 小于等于 0 时移除并播放反馈。

### 6.9 敌方 AI

敌方 AI 当前规则：

- 如果敌方基地存在，每 `0.9-1.55` 秒尝试行动一次。
- 从可连接、可购买地块中选择目标。
- 如果敌方金币不足，则暂不购买。
- 购买后按同样地块翻开规则生成建筑。
