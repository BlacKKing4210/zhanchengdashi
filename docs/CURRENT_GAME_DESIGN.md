...
| 防御/金矿卡牌 | 解锁防御塔时从已配置的防御塔卡中按目标品质选择；若该品质没有防御塔，则按 common -> rare -> epic -> legendary 顺序向下一品质顺延，直到找到可用防御塔；若已到 legendary 仍没有可用塔，则向低品质选择最近一档作为安全兜底；所有品质均未配置防御塔时，该地块变为空地。金矿固定绑定紫色金矿卡 |
...
- 防御塔不沿用营地的向低品质降级规则：目标品质没有已配置防御塔时，按 common -> rare -> epic -> legendary 向下一品质逐级顺延；仅当更高品质全部不可用时，才向低品质选择最近可用塔作为终端兜底；所有品质都没有防御塔时，解锁结果为空地。
...
| defense | 根据开局价格选择对应防御卡池，抽 1 张防御卡；目标品质未配置塔时向下一品质顺延 |
| gold_mine | 固定生成金矿 |
| home_base | 固定生成大本营 |
...
- 防御池抽中的品质若没有对应防御塔，按 common -> rare -> epic -> legendary 向更高品质逐级寻找；更高品质全部不可用时，才选择最近的低品质塔保底；所有品质均无塔时生成空地。
