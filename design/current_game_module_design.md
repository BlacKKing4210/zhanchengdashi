...
- 未建造地块只保留可建造图标与金币价格，不显示“未建”文字标识。
- 玩家从暂停菜单主动点击“退出”时立即返回大厅，不进入结算，也不发放抽卡券、段位、排名等奖励。
- 本地玩家摧毁其他势力（玩家或电脑）的大本营时播放胜利音效；本地玩家的大本营被其他势力摧毁时播放失败音效。结算音效不得与该次大本营事件重复播放。
...
- 自动战斗：建筑自动产出单位，单位自动寻找敌对动物或敌方建筑；两者均可成为推进与攻击目标，不再把空地地块作为攻击目标。
...
- 当前已有本地段位与镜像数据库，使用 `user://rank_mirror_db.json` 保存玩家段位、星数、胜负和胜利镜像；金币、抽卡券、卡牌数量和编组仍主要在每次运行时从初始状态开始。
...
当前原型使用星数制段位。玩家资料保存当前段位 `rank_key` 和星数 `stars`，所有游戏页面只显示“段位 + 星数”，不显示隐藏分或数值变化。
...
| 触发 | 数据规则 |
| --- | --- |
| 任意胜负结算 | 胜利固定 +1 星，失败固定 -1 星，并按段位上限处理晋级/降级 |
| 玩家胜利 | 将本局开战时所在段位、星数、8 张出战卡和卡牌等级记录为一份胜利镜像 |
| 玩家失败 | 只更新星数和胜负统计，不写入胜利镜像 |
...
- 待解锁地块只知道站点类型和价格，不知道本次解锁后的品质；动物营地、防御塔、金矿在未解锁时统一使用中性色简化图标。
...
| 初始拥有 | `_init_player_collection()` | 当前给前 8 张普通动物、金矿卡和绿色防御塔卡，后续可做新手指定卡组 |
