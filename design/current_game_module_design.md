...
- 自动战斗：建筑自动产出单位，单位自动寻找目标、移动、攻击建筑或占领空地。
...
- 玩家初始只拥有 1 个已解锁地块：己方大本营，坐标 `(3, 11)`。
- AI 初始只拥有 1 个已解锁地块：敌方大本营，坐标 `(3, 1)`。
- 其他地块均为未解锁状态，但会在开局生成固定的隐藏类型和价格。
...
- 大本营：自动生成基础单位，玩家默认使用编组第一张卡，敌方默认使用狼。
...
1. 若死亡则从单位列表移除。
2. 搜索最近目标地块。
3. 若目标在攻击距离内且冷却结束，则造成伤害。
4. 若不在攻击距离内，则向目标移动。
5. 攻击空地时只改变 `occupier` 占领颜色，不会解锁地块。
...
| 建筑 | 当前生命 | 生产/攻击规则 |
| --- | ---: | --- |
| 大本营 | 520 | 玩家 4.6 秒产兵，AI 4.2 秒产兵 |
| 营地 | 145 | 按绑定卡牌召唤间隔产兵 |
| 大厅 | 210 | 按绑定卡牌召唤间隔产兵 |
| 防御塔 | 180 | 1.1 秒刷新攻击逻辑，当前每次命中伤害 22 |
...
| AI 默认单位 | 狼 | 可按关卡/难度读配置表 |
| AI 金币 | 和玩家同起点同收入 | 可做简单难度倍率 |
