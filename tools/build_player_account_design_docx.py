...
set_font(subtitle.add_run("安装 ID 自动登录、本机档案切换与服务端权威进度"), 13, color="4B5563")
...
sections = [
    ...
    ("2. 默认设备账号流程", [
        ...
        "账号密码手动登录成功后，服务器校验当前安装 ID 与长期令牌，把该账号加入本机档案集合并设为当前档案；客户端不保存账号密码明文。",
    ]),
    ("3. 本机账号切换", [
        ...
        "账号密码手动登录成功后，该账号加入当前安装的可切换集合并成为当前档案；本机仍不保存账号密码。",
    ]),
    ("4. 安全边界", [
        "服务器只保存安装 ID 的 SHA-256 摘要、随机盐和长期令牌哈希，不保存安装 ID 或令牌明文。",
        "客户端只在 user:// 保存随机安装 ID 与随机长期令牌，不保存账号密码；长期令牌错误时服务器统一拒绝认证。",
    ...
    ("6. 页面入口", [
        "原账号与密码注册/登录入口继续保留；手动登录会把当前安装安全绑定到该账号，密码只在服务端保存加盐派生哈希。",
        "主页面点击基地仅打开账号中心，不再负责触发自动登录；可查看 UserID、切换或新建档案、阅读玩家协议并切换音乐与音效。",
    ]),
    ("7. 验收", [
        "全新安装首次连接后得到 UserID、长期令牌和默认服务器资料；已有长期令牌时启动客户端无需点击基地便自动恢复账号。",
        "账号密码手动登录后重启客户端，自动登录到同一 UserID，且本地凭据文件不包含密码。",
