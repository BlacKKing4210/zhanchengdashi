...
def build_document(art_dir=ART):
    ...
    body(doc,"首次获得一种新动物时，先展示对应品质色的卡背，点击卡牌或等待2秒后播放翻开效果，再展示大动物、名称和继续按钮。卡牌下方不显示说明、操作引导或倒计时，演出不显示品质文字、属性或能力说明。所有品质首得均触发，重复动物不触发；十连按结果顺序依次展示，不改变概率、消耗或发卡事务。")
    table(doc,["页面与状态","显示与转换","数据和边界"],[
        ["G01 卡背与翻开","品质色卡背及光效；点击或2秒超时启动同一翻牌，完成后到G02","翻牌须可见收窄与展开，静止卡和美术等比；触发只一次，动画中屏蔽穿透"],
        ["G02 动物特写","放大的正式动物美术、名称与继续按钮；继续到下一位G01","品质仅由颜色表达；详细属性与能力保留在收藏详情；特写只播放一次柔和惊喜音"],
        ["G03 本次抽卡结果","沿用原圆角收藏卡比例与风格；美术等比放大并保留名称","无品质文字、等级、升级数量/进度或按钮；无新动物直接进入，单抽十连一致"],
    ...
    for item in [
        ...
        "四种品质逐个检查背景、光效与正式动物美术；720×1280和360×640下无品质文字，动物等比且更大，名称与继续按钮可读。",
        "单抽和十连结果保持原圆角卡比例；不拉伸美术，无等级、升级进度或品质文字，名称无裁切。",
