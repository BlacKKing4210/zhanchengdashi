...
def build_figures():
    ...
    pages = [
        ...
        ("H02 锁格点击反馈", "无色地块  类型剪影  金币价格", "价格可读：点击校验邻接和余额\n价格不可读：首击居中，不扣款", "居中后再点建造；不足只短提示", "不打开锁格详情；成功赠 1 券；无升级玩法"),
...
def build_document():
    ...
    table(doc,["版本控制","内容"],[
        ["文件状态","实现合同 v1.1；修订与服务器验收以当前回执为准；Figma 正式图稿 Pending；未做 Android 真机验收"],
        ["编号与分类","REQ-20260925-HOME-GACHA-POLISH / F-ZC-HOME-001；IMPLEMENTATION_CONTRACT / MATERIAL；通用模板"],
        ...
        ["实施范围","现有家园入口、解锁、动物表现、真实日收益、账号经济接口和现有阿里云测试服家园更新；不含生产服和设备发行"],
    ],[1.15,5.5])
    table(doc,["版本","日期","更新内容"],[["1.0","2026-09-24","新增家园、最多补领3天、解锁赠券、新地次日收益及新动物演出。"],["1.1","2026-09-25","中性未解锁地块、直接点击建造、按圈数固定外观且无升级玩法；放大建筑；柔化音效；抽卡隐藏升级信息；修复家园服务器与主堡显示。"]],[0.6,1.0,5.05])
    ...
    for s in ["让玩家用现有金币扩展一个可观察、可成长的动物家园。","地块解锁前通过简易剪影看懂类型与金币价格；解锁即时得到一张抽卡券。","动物有昼夜生活节奏；行走、进出建筑、正面心情占多数均能直接看见。","每日登录有稳定城堡基础收益，最多三天补领降低漏登损失。"]: add_bullet(doc,s)
    ...
    table(doc,["层次","玩家结果与规则归属"],[
        ...
        ["扩建","无色地块＋类型剪影＋金币价格 → 价格可读时点击校验邻接/余额 → 一次扣款、拥有地块、券 +1；价格不可读先居中，不扣款；不足只短提示。"],
    ...
    table(doc,["起点","动作或条件","去向与反馈"],[
        ...
        ["H02 锁格点击","价格可读时建造","检查通过回 H01，券 +1；价格不可读先居中；不足或不邻接只短提示，不打开详情"],
        ["H03 已有详情","选择其他已有格","更新详情；H02无详情面板，不保留隐形按钮"],
    ],[1.25,1.65,3.75])
    body(doc,"缩小或拖动地图使锁格金币价格不可读时，第一次点击只将该地块移到画面中心并显示可读价格，不扣款；再次点击直接建造。价格正常可见时仍一次点击建造。")
    ...
    table(doc,["来源","定位与用途"],[
        ["需求来源","docs/HOME_GACHA_POLISH_IMPLEMENTATION.md；docs/receipts/REQ-20260924-HOMESTEAD.md；2026-09-24 用户文字与参考图"],
    ...
    body(doc,"主目标 720×1280，同时检查 360×640 缩小窗口。720 基准布局：顶部资源区 y=0～190，地图 y=190～950，底部详情 y=962～1128，导航保持既有位置。复用页面皮肤、金币与券图标、已解锁地格和动物。锁格无类型底色，仅剪影与价格；已有建筑是地块主体，详情显示类型。")
    table(doc,["元素","显示与交互","来源与异常"],[
        ...
        ["锁定地块","无色底板、类型剪影、金币价格；价格可读时点击即尝试解锁","价格不可读：首击仅居中展示，不扣款，次击建造；不打开详情，不足或不邻接只短提示"],
        ...
        ["地图操作","移除缩放/回中按钮和连接横条；拖动、手势浏览地图","删除旧按钮命中区；双指操作不误触购买；弹窗拦截输入"],
    ],[1.2,2.85,2.6])
    doc.add_heading("9.1 必须覆盖的页面状态",level=2)
    body(doc,"主页：首次仅城堡、已扩展、最大圈数、昼间、夜间；详情仅已拥有；锁格点击覆盖足额、不邻接、金币不足、请求中，以及缩小或拖动后价格不可读的首击居中与次击建造；收益：1 天/3 天、空列表但有城堡、最大建筑量分页、领取成功、同日已领、跨日；连接：加载、失败、超时重查、账号切换、重进恢复。")
    ...
    body(doc,"家园沿用账号金币和券，居民读取卡牌；不改战斗金币、升级费用或抽卡概率。家园请求不信任自报价格、地块、时间或奖励；但现有整包客户端钱包提交机制仍存在，本次不是完整防作弊改造。本地修订及阿里云现有测试服的家园更新均以最新实施回执为准；Android 真机尚未验证。")
    ...
    body(doc,"本次修订在原有经济行为上进行。以下验收记录保留历史基线，v1.1最新结果以任务回执为准，证据目录 temp/qa/home-polish-20260925；历史经济回归见 temp/qa/home-20260924。Windows 引擎输入、GPU 渲染及隔离存储结果不代表阿里云已部署或 Android 真机已通过。")
    table(doc,["ID","场景与通过条件","证据"],[
        ["H-Q01","真实鼠标/触摸点击左下入口打开家园；返回与重进正确","运行输入记录"],
        ["H-Q02","第一至第八圈价格范围、每圈 ×3 基准、目录稳定；价值单调并封顶","经济规则测试"],
        ["H-Q03","价格不可读首击居中不扣款、次击建造；正常可读一击建造；非邻接/不足金币拒绝；准确扣款并立即 +1 券，不重复发","事务与运行测试"],
    ...
    table(doc,["门禁","当前状态","责任人与关闭条件"],[
        ...
        ["运行逻辑与玩家可见验收","以v1.1回执为准","docs/receipts/REQ-20260925-HOME-GACHA-POLISH.md；保留历史基线，新的截图与输入测试独立记录"],
        ["阿里云测试服家园更新","用户已授权","现有 OpenClaw-ejrr 测试服；仅更新家园功能；结果见最新部署回执"],["生产服与设备发行","本次未执行","不属于本次家园测试服更新范围"],
    ],[1.6,1.45,3.6])
    ...
    body(doc,"2026-09-25 用户新增请求：抽卡首次获得一种新动物时，先展示品质揭晓页，玩家点击或等待 2 秒后进入动物特写页；背景与光效采用该动物的品质色，所有品质的首得动物均触发；重复动物不触发。十连中有多个新动物时，按本次结果顺序依次展示。此改动独立于家园收益规则，不改变抽卡概率、消耗或卡牌发放。")
    table(doc,["页面与状态","显示与转换","数据和边界"],[
        ...
        ["G03 本次抽卡结果","所有新动物演出结束后进入抽卡结果；结果卡只显示美术、名称、品质，不显示等级或升级材料进度","没有新动物时直接走原结果；重复卡不加入首次获得演出队列"],
        ["中断与连续输入","点击切页与 2 秒计时竞争时只推进一次；清理上一页计时和输入","奖励已在原抽卡事务中写入；演出重复、跳过、退出或重进均不重复发卡"],
    ],[1.35,2.65,2.65])
    body(doc,"v1.1补充：仅已拥有动物不触发首得流程。首次获得任意品质动物，特写出现时播放一次独立柔和惊喜音，点击跳过与2秒自动推进行为一致。普通结果卡不显示等级角标、升级数量/进度；编组和动物详情继续显示原升级信息。")
