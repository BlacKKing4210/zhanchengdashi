...
func _test_ready_start_and_running_lock() -> void:
	...
	_expect_error(registry.start_room(201), "players_not_ready", "host cannot start before the guest is ready")
	var host_ready_result = registry.set_ready(201, false)
	_expect_true(bool(host_ready_result.get("ready", false)), "host remains implicitly ready even if an old client requests false")
	...
	_expect_true(bool((host_snapshot.get("slots", []) as Array)[0].get("ready", false)), "host slot is published as implicitly ready")
	_expect_true(bool(host_snapshot.get("all_ready", false)), "full room reports all_ready when every guest is ready")
	_expect_true(bool(host_snapshot.get("can_start", false)), "host may start without a separate ready action")
