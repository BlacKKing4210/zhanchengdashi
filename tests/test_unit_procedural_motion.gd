...
func _test_unit_value_feedback() -> void:
	...
	app.set("effects", [])
	var speed_before = float(all_units[unit_index]["speed"])
	app.call("_show_unit_value_feedback", unit_index, "speed", 20.0)
	_expect_true(_last_effect("unit_value").is_empty(), "movement speed never creates overhead value feedback")
	_expect_false(bool(app.call("_is_unit_value_feedback_visible", "speed")), "online-style speed effects are rejected by the draw guard")
	app.call("_add_aura_speed", BoardRules.PLAYER, Vector2.ZERO, 1.20, true)
	all_units = app.get("units")
	_expect_close(float(all_units[unit_index]["speed"]), speed_before * 1.20, "percentage speed aura still changes battle movement speed")
	_expect_true(_last_effect("unit_value").is_empty(), "percentage speed aura creates no overhead feedback")
	var percentage_speed = float(all_units[unit_index]["speed"])
	app.call("_add_aura_speed_flat", BoardRules.PLAYER, Vector2.ZERO, 20.0, true)
	all_units = app.get("units")
	_expect_close(float(all_units[unit_index]["speed"]), percentage_speed + 10.0, "flat speed aura still applies the battle speed multiplier")
	_expect_true(_last_effect("unit_value").is_empty(), "flat speed aura creates no overhead feedback")
