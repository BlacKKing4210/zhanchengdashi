...
func _test_all_classic_maps() -> void:
	for map_id in MultiplayerRules.map_ids_for_size(1):
		...
		_expect_equal(_territory_count(tiles, BoardRules.PLAYER), 37, "%s gives the player 37 cells after frontier optimization" % map_id)
		_expect_equal(_territory_count(tiles, BoardRules.ENEMY), 37, "%s gives the enemy 37 cells after frontier optimization" % map_id)
